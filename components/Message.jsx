				...
				<div className='flex w-screen h-screen overflow-y-hidden'>
						...
						<h1 className='absolute text-white ml-20 mt-4'>Username</h1>
							<div className='absolute flex flex-col gap-8 ml-4 mt-36 text-black'>
								<div>
									  <button className='flex flex-row gap-2 cursor-pointer shadow-xl text-white border-2 rounded-full p-4 shadow-black text-2xl ' onClick={handleMessage1} ><FaComments/>Messages</button>
									</div>
	  ...
	  {data.map((item) => (
		<li key={item._id}>
			...
			<div className="flex flex-row gap-96">
				<h3>{item.titi}</h3>
			<button className='ml-auto mt-2 text-red-500' onClick={() => { settiti(item.titi); handlefai() }}><FaHeart/></button>
			</div>
			<p className='ml-12 text-black' >{item.cont}</p>
			</div>
		</li>
	  ))}
	  </ul>
